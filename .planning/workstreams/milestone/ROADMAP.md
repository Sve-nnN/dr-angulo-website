...
- [x] 08-11-PLAN.md — Ola 2, último plan: `/blog/miedo-a-operarte-de-la-columna-5-cosas-que-debes-saber` conserva su URL y pasa a pelear `cirugía de columna` con 1747 palabras y las 20 secciones del paquete, con `h1` propio y los seis enlaces de la matriz. `relatedService` se queda en `hernia-discal` porque la guía de cirugía mínimamente invasiva todavía no existe como ruta. Cerrado el 2026-08-13. **Ola 2 completa (08-06 a 08-11)**
- [x] 08-15-PLAN.md — Ola 3, primer plan: `/servicios/escoliosis` se renombra a `/servicios/escoliosis-y-deformidades` y publica las 19 secciones del copy aprobado, 2615 palabras. El slug viejo responde 308 permanente desde el bloque `redirects()` que este plan estrena en `next.config.ts`. `bannerAfterSectionId: "que-es--escoliosis-dorsal"`, porque `que-es` arrastra cinco subsecciones y el banner por defecto caía en el 38.4 por ciento. Cierra SVC-03, que había quedado sin plan que lo reclamara. Cerrado el 2026-08-13
- [x] 08-16-PLAN.md — Ola 4: entra `/servicios/cirugia-minimamente-invasiva`, la quinta hija del silo y la única URL nueva del set, con las 18 secciones del paquete y 1794 palabras. El hub `/servicios` publica sus 16 secciones y 1191 palabras con el `ContentBody` del 08-09, conservando las cuatro anclas de v1.0 que el inicio enlaza, y su `h1` pasa a "Cirujano de columna en Lima". `SITEMAP_TOTAL` sube a 22 en las tres puertas. Cierra la parte de SVC-05 que había quedado sin dueño. Cerrado el 2026-08-13
- [x] 08-12-PLAN.md — Ola 5: nace `/blog/artrosis`, la primera URL de blog que el paquete manda crear en vez de reescribir, con las 20 secciones del copy aprobado y 1989 palabras. Es el primer post del sitio sin `relatedService`: la matriz de la fase 14 no le da mención inversa hacia ninguna guía y su salida al silo es el hub `/servicios`, así que su entrada del `MANIFEST` va sin `linksTo`. `bannerAfterSectionId: "que-es--artrosis-de-rodilla"`, porque `que-es` arrastra seis subsecciones. `SITEMAP_TOTAL` sube a 23 en las tres puertas. Cerrado el 2026-08-13
- [x] 08-13-PLAN.md — Ola 6: nace `/blog/lumbalgia`, la segunda y última URL de blog del paquete, con las 19 secciones del copy aprobado y 1737 palabras. A diferencia de artrosis sí declara `relatedService: "hernia-discal"`, que es la mención inversa que le asigna la matriz, y enlaza a esa guía con el anchor literal `hernia discal lumbosacra tratamiento`. `bannerAfterSectionId: "sintomas"`, la única frontera de nivel 2 dentro de la ventana de POS-01. `SITEMAP_TOTAL` sube a 24 en las tres puertas, el máximo de la fase. Cerrado el 2026-08-13. **Todo el contenido nuevo o reescrito de la fase está publicado**
- [x] 08-14-PLAN.md — Ola 7, checkpoint bloqueante: los dos posts absorbidos (hernia-discal-o-dolor-de-espalda-como-diferenciarlos, estenosis-espinal-que-es) 301 permanentes hacia sus guías destino, con aprobación explícita de Juan tras verificar las 16 filas de material absorbido sobre el HTML construido. `SITEMAP_TOTAL` baja a 22, el número final del silo. Verificado con `curl -I` contra el build real. Cerrado el 2026-08-13. **Cierra todo el cuerpo de contenido de la fase; quedan solo 08-17 a 08-19 (sedes + preguntas-frecuentes)**
- [x] 08-17-PLAN.md — Ola 8: `src/content/location-pages.ts` se parte en un módulo por sede y `LocationPage` admite cuerpo largo, con la plantilla de sede renderizando el `ContentBody` del 08-09 como su cuarta superficie. La Clínica Ricardo Palma y la Clínica Padre Luis Tezza publican las 16 secciones de su copy aprobado, 1656 y 1553 palabras, con `h1` del dataset y sin afirmar ninguno de sus cuatro datos operativos pendientes. `check-content.mjs` gana el esqueleto `ficha-de-sede`. `SITEMAP_TOTAL` no se mueve: las cuatro rutas ya existían. Cerrado el 2026-08-13
- [x] 08-18-PLAN.md — Ola 9: el consultorio privado de Surco y Sanna La Molina publican las 20 secciones de su copy aprobado, 1673 y 1747 palabras, con `h1` del dataset. El consultorio sigue siendo la única sede que ofrece el chat del doctor y ninguna de las dos afirma sus seis datos operativos pendientes, con lo que quedan cubiertos los diez del paquete de sedes. Las cuatro fichas entran al `MANIFEST` de `check-content.mjs`, que pasa a medir 13 rutas. `SITEMAP_TOTAL` sigue en 22. Cerrado el 2026-08-13. **Cierra las cuatro sedes; queda solo 08-19**
- [ ] 08-19-PLAN.md — Lo que queda: preguntas frecuentes
...
| 8. Silo clínico: servicios y blog profundo | v1.1 | 6/18 | In progress (reabierta para el paquete on-page de v1.2) | - |
