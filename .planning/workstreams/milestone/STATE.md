...
- **El title de una ruta del paquete va como `title: { absolute }`.** El layout raíz declara `title.template` con el sufijo ` | Dr. Juan Angulo`, 19 caracteres que Next pega detrás de todo title hijo. Los conteos del paquete v1.2 son del title final que ve Google, así que con la forma suelta varias rutas se pasaban de 60. El `template` se queda igual porque `/privacidad` y `/not-found` no están en el paquete y con el sufijo puesto siguen dentro del contrato.
...
Stopped at: Plan 10-02 cerrado. Las 13 rutas dinámicas vivas (4 sedes, 5 guías de servicio y 4 posts) sirven el title y la meta exactos del paquete on-page de v1.2, medidos sobre el HTML del build y con las rutas sacadas del sitemap, no de una lista escrita a mano: los 26 conteos coinciden uno a uno con los del handoff, el title más largo mide 50 y la meta más larga 142. El cruce previo entre el paquete y `src/content/` dio limpio en las dos direcciones, así que la fase 8 quedó completa: no falta ninguna ruta ni sobra ninguna sin fila. Los dos posts que la fase 8 apagó viven solo como 301 en `next.config.ts`, sin metadata propia. Salieron tres desvíos, los tres arreglados dentro del plan: los `generateMetadata` de los tres `[slug]` arrastraban el sufijo del `title.template` y empujaban 10 de 13 titles sobre los 60 caracteres, resuelto con `title: { absolute }` igual que en 10-01; `check-sedes.mjs` tenía escritos a mano los cuatro titles viejos y ahora espera los nuevos, sin la constante `TITLE_SUFFIX`; y los tres módulos planos que el plan nombraba ya estaban partidos en carpetas por la fase 8, así que se editó la carpeta equivalente sin revertir el split. Las tarjetas OG de `/servicios/hernia-discal` y `/sedes/clinica-tezza` se descargaron del build y se miraron: el title nuevo entra completo con su eyebrow y sus credenciales. `tsc`, `lint`, `build`, `check-content.mjs`, `check-sedes.mjs` y `seo:check` en verde. Lo que sigue: el plan 10-03, la puerta permanente de 60/155 y el arreglo de REQUIREMENTS.md. Es lo único que queda de la fase 10 y de todo el trabajo de SEO-08 y SEO-09 en v1.1.

Contexto previo, plan 10-01: Las 9 rutas base con metadata estática (`/`, `/agendar`, `/blog`, `/contacto`, `/preguntas-frecuentes`, `/sedes`, `/servicios`, `/sobre-el-doctor`, `/testimonios`) sirven el title y la meta description exactos del paquete on-page de v1.2, medidos sobre el HTML prerenderizado: los 9 conteos coinciden uno a uno con los del handoff, el más largo mide 48 y ninguna meta pasa de 155. El detalle que decidía el plan era el sufijo del `title.template` del layout, resuelto con `title: { absolute }` en cada ruta. El layout además dejó de heredar la description larga de `siteConfig`: su `og:description` bajó a 127 caracteres y `title.default` pasó a ser el title de la portada. `site-config.ts` no se tocó porque alimenta el JSON-LD y `/llms.txt`. Las tarjetas OG de `/` y `/blog` se descargaron del build de producción y se miraron: el title nuevo entra completo, más corto que el anterior. `check-content.mjs`, `check-sedes.mjs` y `seo:check` en verde. Lo que sigue: el plan 10-02, que aplica las 13 filas restantes del handoff a las rutas de contenido dinámico. SEO-08 sigue Pending hasta que ese plan cierre.
Sesión anterior: Fase 8 cerrada con 08-19, el último de sus 18 planes. `/preguntas-frecuentes` deja de ser un acordeón de preguntas cortas y publica la guía clínica del paquete: 19 secciones, 8 de nivel 2 con el esqueleto canónico de `guia-clinica`, 2183 palabras de cuerpo medido y el `h1` del dataset, "Dudas frecuentes antes de la consulta". La decisión de fondo del plan era qué hacer con el acordeón de v1.0, que en disco tenía diez preguntas y no once: siete se retiraron porque su respuesta ya vive en el texto sellado y tres sobreviven, recuperación tras cirugía, plazo de vuelta al trabajo y cómo se saca la cita en cada sede, porque el paquete no las responde. La comparación pregunta por pregunta está transcrita en el SUMMARY. `FaqJsonLd` pasa a armarse con las cuatro subsecciones de la sección `preguntas-frecuentes` del paquete más las tres supervivientes, siete preguntas en total, y el HTML sigue emitiendo `FAQPage`. `faq.ts` lo consumen tres superficies y las tres siguen vivas: esta ruta, el bloque de preguntas del inicio (que ahora muestra las tres supervivientes, cambio visible que conviene mirar) y `llms.txt`, que bajó de 161 a 154 líneas. `check-content.mjs` estrena el esqueleto `guia-clinica` con sus ocho claves, mide 14 rutas y `SITEMAP_TOTAL` sigue en 22. Lo que sigue: la ronda de verificación de la fase 8 y después la fase 10, que reescribe title y meta de todas estas rutas.

Anterior: Ola 9 cerrada con 08-18, y con ella las cuatro fichas de sede. `/sedes/consultorio-privado` y `/sedes/sanna-la-molina` publican sus 20 secciones y 1673 y 1747 palabras, transcritas del dataset sellado igual que las dos del 08-17, con el `h1` del dataset y el `title` todavía esperando a la fase 10. El consultorio sigue siendo la única sede que ofrece el chat del doctor, y la puerta de sedes lo confirma sobre el HTML real en las cuatro. Los seis datos operativos pendientes de estas dos, los dos bloques de seguros y convenios, el estacionamiento de la torre de Surco, el piso del centro clínico de La Molina y los dos precios de consulta, siguen sin afirmarse y están transcritos en el SUMMARY: con eso quedan cubiertos los diez pendientes del paquete de sedes. Las dos rutas entraron al `MANIFEST` de `check-content.mjs`, que ahora mide 13 rutas y las cuatro sedes, y `SITEMAP_TOTAL` sigue en 22. La posición del banner se calculó por ratio acumulado de palabras en vez de elegirse a ojo, que es lo que en el 08-17 dejó una sede rozando el piso del rango. Lo que sigue: el 08-19, preguntas frecuentes, es el último plan abierto de la fase.

Antes de eso: Ola 8 abierta con 08-17. `src/content/location-pages.ts` es ahora un directorio con `types.ts`, `index.ts` y un módulo por sede, y el archivo suelto quedó como barril delgado, así que los diez consumidores no tocaron su import. `LocationPage` admite `sections` y `outboundLinks`, más `ctaBanner`, `publishedAt`, `updatedAt` y `bannerAfterSectionId`, los cuatro opcionales y agregados porque entrar al `MANIFEST` de la puerta de contenido obliga a firma, banner e índice. La plantilla de sede es la cuarta superficie que usa el `ContentBody` del 08-09, y todo lo nuevo está condicionado a que la sede tenga cuerpo: con `sections` vacío las cuatro sedes renderizaron el mismo conteo de palabras que antes del plan. `/sedes/clinica-ricardo-palma` y `/sedes/clinica-tezza` publican sus 16 secciones y 1656 y 1553 palabras, con el `h1` del dataset (el `title` sigue esperando a la fase 10) y sin afirmar ninguno de sus cuatro datos operativos pendientes: los dos pisos y los dos bloques de seguros y convenios siguen sin confirmar y están transcritos en el SUMMARY. `MidContentCta` ganó `withWhatsApp`, porque el banner de una sede de clínica no puede ofrecer el chat del doctor. `check-content.mjs` declara el esqueleto `ficha-de-sede` y `SITEMAP_TOTAL` sigue en 22. Lo que sigue: el 08-18 publica el consultorio privado y Sanna La Molina sobre esta misma forma, y el 08-19 es preguntas frecuentes.

Antes: Ola 6 cerrada con 08-13, y con ella todo el contenido nuevo o reescrito de la fase. `/blog/lumbalgia` existe y publica las 19 secciones del paquete con 1737 palabras de cuerpo: 8 de nivel 2 y 11 de nivel 3, contra las 15 que el plan había contado, porque el plan contó de menos y manda el dataset. A diferencia de artrosis, este post sí declara `relatedService: "hernia-discal"`, que es la mención inversa que la matriz le asigna, y su enlace de salida usa el anchor literal `hernia discal lumbosacra tratamiento` en vez de la frase que la plantilla arma sola. Su banner cuelga de `sintomas`, la única frontera de nivel 2 que cae dentro de la ventana de POS-01. `SITEMAP_TOTAL` está en 24 en las tres puertas y el sitemap real tiene 24 URLs: es el máximo de la fase. El enlace que el 08-12 dejó apuntando a un 404 ya resuelve. Lo que queda: el 08-14 devuelve el conteo a 22 con las redirecciones de los dos posts absorbidos, y 08-17 a 08-19 son sedes y preguntas frecuentes.

Y antes: Ola 5 abierta con 08-12. `/blog/artrosis` existe y publica las 20 secciones del paquete con 1989 palabras: 8 de nivel 2 y 12 de nivel 3, contra las 19 que el plan había contado, porque el plan se saltó `que-es--artrosis-mano` y manda el dataset. Es el primer post del sitio sin `relatedService`. La matriz de la fase 14 no le asigna mención inversa hacia ninguna guía, así que sale al silo por el hub `/servicios` y su entrada del `MANIFEST` va sin `linksTo`, que es el caso para el que el 08-05 dejó el campo opcional. Su banner cuelga de `que-es--artrosis-de-rodilla`, porque `que-es` arrastra seis subsecciones y la única frontera de nivel 2 cercana cae en el 41.9 por ciento. `SITEMAP_TOTAL` está en 23 en las tres puertas y el sitemap real tiene 23 URLs. Queda un enlace de salida hacia `/blog/lumbalgia`, que todavía responde 404: lo cierra el 08-13, que además sube el conteo a 24. Después el 08-14 lo devuelve a 22 con las redirecciones, y 08-17 a 08-19 son sedes y preguntas frecuentes.

Y más atrás: Ola 4 abierta y cerrada en su parte de silo con 08-16. `/servicios/cirugia-minimamente-invasiva` existe: es la única URL nueva del set, publica las 18 secciones del paquete con 1794 palabras y su banner cuelga de `que-se-atiende--endoscopia-espinal`, porque las siete subsecciones del primer bloque dejan la única frontera de nivel 2 fuera de la ventana de POS-01. El hub `/servicios` publica sus 16 secciones y 1191 palabras con el `ContentBody` del 08-09, entre la rejilla de tarjetas y el catálogo heredado, y conserva las cuatro anclas de v1.0. Su `h1` pasó a "Cirujano de columna en Lima"; el title y la meta siguen siendo de la fase 10. `SITEMAP_TOTAL` está en 22 en las tres puertas y el sitemap real tiene 22 URLs. El enlace que el 08-15 dejó apuntando a la cirugía mínimamente invasiva ya resuelve. Lo que sigue: 08-12 y 08-13 suben el conteo a 23 y 24 con los posts nuevos, el 08-14 lo devuelve a 22 con las redirecciones, y 08-17 a 08-19 son sedes y preguntas frecuentes.
